    ...
    <>
      <div className={`popup popup_type_${props.name} ${props.isOpen ? 'popup_opened' : ''}`}>
        <div className={`popup__container ${props.name == 'update-avatar' || props.name == 'delete' ? `popup-${props.name}__container` : ''}`}>
          ...
          <form className={`form popup__form popup__form-${props.name}`} name={`form-${props.name}`} method="get" noValidate>
            {props.children}
            <button className={`button popup__button popup__btn-${props.name}`} type="submit">{props.buttonText}</button>
          </form>
        </div>
      </div>
    </>
